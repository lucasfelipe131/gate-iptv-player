...
test("carrega a camada visual profissional depois dos estilos base", () => {
  ...
  const polish = indexHtml.indexOf("/ui-polish.css?v=0.6.5");
  assert.ok(base >= 0);
  ...
  assert.match(indexHtml, /pro-ui\.js\?v=0\.6\.5/);
  assert.match(proJs, /ui-polish\.css\?v=0\.6\.5/);
});
...
test("service worker publica os novos recursos sem usar o cache antigo", () => {
  assert.match(serviceWorker, /gate-player-v21-clean-tv-settings-0-6-5/);
  for (const asset of ["pro-ui.css", "ui-polish.css", "pro-ui.js", "web-ui.css", "app.js"]) {
    assert.match(serviceWorker, new RegExp(`${asset.replace(".", "\\.")}\\?v=0\\.6\\.5`));
  }
