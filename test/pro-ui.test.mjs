...
test("carrega a camada visual profissional depois dos estilos base", () => {
  ...
  assert.match(proJs, /document\.head\.appendChild\(link\)/);
});
...
test("service worker publica os novos recursos sem usar o cache antigo", () => {
  assert.match(serviceWorker, /gate-player-v18-tv-ui-2-1/);
  ...
  assert.doesNotMatch(serviceWorker, /gate-player-v17-web-ui-2/);
});
